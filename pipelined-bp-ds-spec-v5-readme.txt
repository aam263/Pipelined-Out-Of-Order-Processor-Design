...
- in the EX phase, the ROB should not pass any elements to the next buffer while it is being broadcast to - the 
  enqueuing is already dealt with through the request arbitration hardware to the DS controller.
...
Note 3 : the structural hazards between the EX phase and the ID phase can be mitigated by checking to see if the addresses match,
	   ...
	   chosen by the arbitration hardware, need an extra conditions test for each functional unit. Only the brodcast stage is 
	   hazard detected to save clock cycles.
